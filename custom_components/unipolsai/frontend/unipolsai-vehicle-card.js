...
/* shim: normalizza l'export di Leaflet a window.L */
if (!window.L && window.leaflet) { window.L = window.leaflet; }

/* ── UnipolSai Vehicle Card ─────────────────────────────────────────────── */
(function () {
  'use strict';

  // ── Leaflet 1.9.4 CSS (bundled) ──────────────────────────────────────────
  const LEAFLET_CSS_BUNDLED = `/* required styles */
...
`;
  ...
  function _loadLeaflet() {
    ...
    // Leaflet bundled: può esporsi come window.L (v1.x) o window.leaflet (v1.9+)
    ...
    async _init() {
      ...
      try {
        ...
        if (!this.shadowRoot.querySelector('style[data-leaflet]')) {
          ...
          st.setAttribute('data-leaflet', '');
          st.textContent = LEAFLET_CSS_BUNDLED;
